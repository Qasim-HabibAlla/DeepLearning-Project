...
class AttnDecoderRNN(nn.Module):
     ...
     def forward(self, input, hidden, encoder_outputs):
            ...
            output, hidden = self.net(output,
                                      hidden_lstm if self.method == "LSTM" else hidden_gru)  
...
def trainIters(encoder, decoder, learning_rate=0.01,print_every=1000,epochs=15):
  print_loss_total = 0
...
def evaluate(encoder, decoder, max_length=MAX_LENGTH):
  ...
  print("\nModel Accuracy of test set ={}%".format(int(100*(correct_count/len(test)))))
  #return output_test
hidden_size = 256

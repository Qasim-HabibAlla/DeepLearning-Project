...
class EncoderRNN(nn.Module):
      def __init__(self, input_size, hidden_size, n_layers=1, bidirectional=False, method='LSTM'):
...
class AttnDecoderRNN(nn.Module):
     def __init__(self, hidden_size, output_size, n_layers=1, dropout_p=0.1,
                     max_length=MAX_LENGTH, method='LSTM', bidirectional=False):
     ...
     def initHidden(self):
            ...
            if self.method == 'GRU':
                ...
                h_state = torch.zeros(self.n_layers * 1, 1, self.hidden_var, device=device)
                c_state = torch.zeros(self.n_layers * 1, 1, self.hidden_var, device=device)
                hidden = (h_state, c_state)
                return hidden
...
sub_df['sentiment']=classification_test
#sub_df['selected_text']=test_df['text']
#sub_df['sentiment']=test_df['sentiment']
#column_names=['textID','text','selected_text','sentiment']
#test_Df = pd.DataFrame(columns = column_names)
#positive_test=[]
#negitive_test=[]
#netural_test=[]
#positive_test=sub_df.loc[sub_df['sentiment'] == 'positive']
#negitive_test=sub_df.loc[sub_df['sentiment'] == 'negative']
#netural_test=sub_df.loc[sub_df['sentiment'] == 'neutral']
...
#for line in sub_df:
 ...
 #   line=pd.DataFrame(line)
  #  positive_test.append(line)
  #elif line[3]=='negative':
   # line=pd.DataFrame(line)
    #negitive_test.append(line)
  #elif line[3]=='neutral':
   # line=pd.DataFrame(line)
    #netural_test.append(line)
#if len(positive_test)<=len(negitive_test):
 # min=len(positive_test)
#else:
 # min=len(negitive_test)
#for i in range(min):
 # test_Df.append(positive_test[i])
  #test_Df.append(negitive_test)
#if min==len(negitive_test):
 # for j in range(min,len(positive_test)):
  #  test_Df.append(positive_test[j])
#else:
 # for j in range(min,len(negitive_test)):
  #  test_Df.append(negitive_test[j])
#for j in range(len(netural_test)):
 # test_Df.append(netural_test[j])
